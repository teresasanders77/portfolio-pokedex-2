...
var pokemonRepository = (function () {
  ...
  function addListItem(pokemon) {
    ...
    var $button = $(
      '<button type="button" class="btn btn-light btn-lg btn-block" data-target="#exampleModal" data-toggle="modal">' +
        pokemon.name +
        '</button>'
    );
    ...
    return $.ajax(url)
      .then(function (details) {
        ...
        for (var x = 0; x < details.abilities.length; x++) {
          item.abilities.push(details.abilities[x].ability.name);
        }
      })
      .catch(function (e) {
        console.error(e);
